...
export class CasesService {
   ...
   */
  async evidenceBundle(id: string, staff: { id: string; email?: string; firstName?: string; lastName?: string }) {
    const c = await this.prisma.underdeclarationCase.findUnique({
      where: { id },
    ...
    // Watermark identifies the officer who pulled the document, not just the
    // org — a screenshot or photo of a cropped page still carries attribution,
    // since the small "Generated by" footer line can be cropped out but the
    // full-page diagonal watermark can't.
 ...
 *{box-sizing:border-box}
 ...
 /* diagonal confidential watermark across the page — org label + the officer
    who generated the document, so attribution survives a cropped screenshot */

...
export class DraftNoticeTool implements ActionTool {
  ...
  readonly inputSchema = {
    type: 'object',
    properties: { caseId: { type: 'string', description: 'The underdeclaration case id to issue a §35 notice on.' } },
    required: ['caseId'],
  };
  ...
  async commit(payload: Record<string, unknown>, ctx: ToolContext): Promise<CommitResult> {
    ...
    const updated = await this.cases.transition(caseId, { to: CaseStatus.NOTICE_ISSUED }, ctx.staff.id);
